...
class SimpleViewPlayer extends StatefulWidget {
  ...
  SimpleViewPlayer(this.source,
      {this.isFullScreen: false, this.apiToken, this.token});
...
class _SimpleViewPlayerState extends State<SimpleViewPlayer> {
  ...
  void initState() {
    ...
    controller.addListener(() {
      if (controller.value.position > controller.value.duration * .7) {
        if (!isSendedView&&widget.apiToken!=null) {
      ...
      return Container(
        ...
        child: Center(
          child: RaisedButton(
            ...
            child: Text("مشکل در پخش ویدیو! لطفا دوباره تلاش کنید"),
          ),
        ),
      );
    } else {
